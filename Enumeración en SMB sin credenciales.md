...
![[Pasted image 20260727170133.png]]
y si son validas para SMB.

Pero se pudo haber hecho mas fácil con algunos scripts de Nmap para SMB como
"nmap -Pn --script smb-enum-shares -p445 10.10.28.90" lo ultimo es la ip y obviamente varia. Y este comando enumera los recursos compartidos "nmap -Pn --script smb-enum-domains -p445 10.10.28.90" que es para enumerar los dominios, "nmap -Pn --script smb-enum-sessions -p445 10.10.28.90" para enumerar las sesiones, "nmap -Pn --script smb-enum-processes -p445 10.10.28.90" para enumerar los procesos y por ultimo "nmap -Pn --script smb-os-discovery -p445 10.10.28.90" y podemos mandar todos estos scripts uno en uno o todos poniendo un asterisco
"nmap -Pn --script smb-enum-* -p445 10.10.28.90" pero no es lo mas optimo por lo que es mejor es ir poco a poco porque en una CTF esta bien pero dentro del trabajo no por la cantidad de informacion y tardará muchisimo